...
public class ShopModModifyOverrideAllINV extends CustomInvLayout implements Listener
{
	ShopModManager _smm;
	...
	public ShopModModifyOverrideAllINV(Main main, Player player, String name, ItemStack mod_itemStack, Shop shop, String[] answers) 
	{
	...
	void makeInv()
	{
		...
		ItemStack remove_button = setupButton(BUTTON.REMOVE_INF, Material.LAVA_BUCKET, ChatColor.RED + ""+ChatColor.BOLD + "Remove All items from shop".toUpperCase(), 8);
		itemM.addLore(remove_button, ChatColor.YELLOW + "Removes all items, infinity and normal ones!", false);
	...
	void setAnwsers()
	{
		...
		for(int i = 0; i < _answers.length; ++i)
		{
			...
			if(ans != null)
			{
				...
				{
					itemM.removeLore(_inv.getItem(i), "Are you sure to REMOVE ALL INF ITEMS?");
					itemM.removeLore(_inv.getItem(i), "WARNING");
					itemM.removeLore(_inv.getItem(i), "WARNING");
					itemM.removeLore(_inv.getItem(i), "WARNING");
					itemM.addLore(_inv.getItem(i), ChatColor.BLUE + "Are you sure to REMOVE ALL INF ITEMS?"+ " Press"+ChatColor.WHITE+ " M2 "+ ChatColor.BLUE + "to confirm!", false);
					
					itemM.addLore(_inv.getItem(i), ChatColor.RED +""+ChatColor.BOLD+ "WARNING!", false);
					itemM.addLore(_inv.getItem(i), ChatColor.RED +""+ChatColor.BOLD+ "WARNING!", false);
					itemM.addLore(_inv.getItem(i), ChatColor.RED +""+ChatColor.BOLD+ "WARNING!", false);
	...
	boolean confirm()
	{
		...
		for(int i = 0; i < _inv.getContents().length; ++i)
		{
			...
			if(s != null)
			{
				for(int shop_idx = 0; shop_idx < _shop.getShopStacks().size(); ++shop_idx)
				{
					ItemStack shop_item = _shop.getShopStacks().get(shop_idx);
					if(shop_item != null && _shop.isStackInf(shop_item))
					{
						...
						case CUSTOM_AMOUNT:
							...
							if(str != null)
							{
								...
								if(itemM.isDigit(str))
								{
									...
									if(amount == _shop.getPDCustomAmount(shop_item))
									{
										continue;
									}
									
									_shop.setPDCustomAmount(shop_item, amount);
							...
							{
								if(_shop.removePDCustomAmount(shop_item))
						...
						case SET_PERMISSION:
							...
							if(str != null)
							{
								if(str.equalsIgnoreCase(_shop.getPDCustomPermission(shop_item)))
								{
									continue;
								}
								
								
								_shop.setPDCustomPermission(shop_item, str.toLowerCase());
								_player.sendMessage(perm_str+" set to: "+ChatColor.GOLD +str.toLowerCase());
								
							}else
							{
								if(_shop.removePDCustomPermission(shop_item))
						...
						case SET_DELAY_AMOUNT:
							...
							if(str != null)
							{
								...
								if(delay_amount_str.length > 1  && itemM.isDigit(delay_amount_str[0]) && itemM.isDigit(delay_amount_str[1]))
								{
									...
									if(_shop.getPDCustomAmount(shop_item) == null)
									{
										int c_a = _shop.getShopStackAmount(shop_item);
										_shop.setPDCustomAmount(shop_item, c_a);
									...
									if(_shop.getPDCustomStockDelay(shop_item) != null 
											&& _shop.getPDCustomStockAmount(shop_item) != null
											&& d_delay == _shop.getPDCustomStockDelay(shop_item) 
											&& d_amount == _shop.getPDCustomStockAmount(shop_item))
									{
										continue;
									}
									_shop.setPDCustomStockDelay(shop_item, d_delay);
									_shop.setPDCustomStockAmount(shop_item,d_amount );
							...
							{
								_shop.removePDCustomStockDelay(shop_item);
								if(_shop.removePDCustomStockAmount(shop_item))
						...
						case SET_CUSTOM_PRICE:
							...
							if(str != null)
							{
								...
								if(prices_str.length > 2 && itemM.isDigit(prices_str[0]) && itemM.isDigit(prices_str[1]) && itemM.isDigit(prices_str[2]))
					            {
									...
									{
										_shop.setPDCustomPrice(shop_item, ds[0]+":"+ds[1]+":"+ds[2]);
							...
							{
								if(_shop.removePDCustomPrice(shop_item))
						...
						case SET_WORLDS:
							...
							if(str != null)
							{
								...
								_shop.setPDCustomWorlds(shop_item, worlds_str);
								_player.sendMessage(custom_worlds_str+" set to: "+ChatColor.GOLD + worlds_str);
							}else
							{
								if(_shop.removePDCustomWorlds(shop_item))
						...
						case SET_CAN_BE_SOLD:
							...
							{
								_shop.removePDCustomCanSoldBack(shop_item);
								_player.sendMessage(custom_canSold_str+" set to: "+ChatColor.GOLD + "true");
							}else
							{
								_shop.setPDCustomCanSoldBack(shop_item);
						...
						case SET_DISTANCE:
							...
							if(str != null)
							{
								...
								_shop.setPDCustomSoldDistance(shop_item, full_str);
								_player.sendMessage(custom_distance_str+" set to: "+ChatColor.GOLD + full_str);
								
							}
							else
							{
								if(_shop.removePDCustomSoldDistance(shop_item))
									_player.sendMessage(custom_distance_str+ ChatColor.RED + ": removed");
							}
							if(fa)
							{
								_shop.removePDCustomSoldDistance(shop_item);
						...
						case SET_TIME_SELL:
							...
							if(str != null)
							{
								...
								if(sellTimes.length > 1 && itemM.isDigit(sellTimes[0]) && itemM.isDigit(sellTimes[1]))
								{
									...
									{
										_shop.setPDCustomTimeSell(shop_item, time1+" "+time2);
							...
							{
								if(_shop.removePDCustomTimeSell(shop_item))
		...
		{			
			...
			if(e.getClick() == ClickType.LEFT)
			{
				...
				case CUSTOM_AMOUNT:
					...
					conv = cf.withFirstPrompt(new ConvPromptModModifyOVERRIDE_ALL_INV(_main, _player, orginal_item, _shop,_answers, slot,question)).withLocalEcho(true).buildConversation(_player);
					conv.begin();
				...
				else if(e.getClick() == ClickType.RIGHT)
			{
				...
				case REMOVE_INF:
					...
					{
						_shop.removeALLItemsFromShopNEW();
						_player.sendMessage(ChatColor.GOLD + "Item(s) has been removed");
